...
This repository starts with a small onboarding foundation and Spooky v0,
...
After setup, open
[notebooks/01_glossary_qa.ipynb](notebooks/01_glossary_qa.ipynb) for Spooky
v0. It searches a pinned public study-group glossary with two deterministic
...
local start command, and browser boundary. The contained Cloud Run deployment
runbook is [docs/spooky-cloud-run.md](docs/spooky-cloud-run.md). It wraps the
existing Spooky agent without exposing ADK's session or event APIs to a
browser.
...
- [notebooks/01_glossary_qa.ipynb](notebooks/01_glossary_qa.ipynb) is Spooky
  v0: a pinned-glossary QA demo using the shared ADK server and session.
